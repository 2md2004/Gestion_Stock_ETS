...
const Vente = () => {
  ...
  // ✅ Calcule debut/fin selon la période choisie
  ...
  const loadPage = async (page) => {
    setLoading(true);
    try {
      const { debut, fin } = getDateRange(periodeFilter);
      const data = await getVentesPerPage(page, 7, "date", debut, fin);
  ...
  // ✅ Recharge à chaque changement de période
  useEffect(() => {
    loadPage(0);
  ...
  return (
    <div className="container">
      ...
      <div className="row">
        ...
        <div
          className="col-md-12 mt-3 shadow-sm p-0 bg-white"
                        ...
                        {resultats.map((vente) => {
                          return (
                            <tr key={vente.id} className="border-bottom">
                              ...
                              <td>
                                  {vente.nomClient || vente.prenomClient
                                    ? `${vente.prenomClient ?? ""} ${vente.nomClient ?? ""}`.trim()
                                    : "Non spécifié"}
                                </td>
                              <td>
                              ...
                              <tr key={vente.id} className="border-bottom">
                                ...
                                <td>
                                  <div className="d-flex align-items-center">
                                    <i className="bi bi-currency-franc me-2 text-success"></i>
                                    <span className="fw-semibold">
                                      {vente.montantTotal?.toLocaleString()}{" "}
                                      FCFA
                                    </span>
                                  </div>
                                </td>
